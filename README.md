...
本项目是一个本地 Web 版 B站下载器 MVP。当前实现覆盖单链接核心闭环，并在阶段二 2.1 增加批量文本解析和自定义文件名，在 2.2 增加字幕下载最小闭环。
...
- 音频输出支持 `m4a` 和 `mp3`。
- 字幕输出支持 `srt` 时间轴字幕和 `txt` 纯文本字幕。
- 视频容器支持 `mp4` 和 `mkv`。
- 批量文本解析支持多行输入、50 条上限、自动去重和逐行资源设置。
- 支持单链接和批量逐行自定义文件名；未填写时默认使用解析标题命名。
...
- 同时下载任务数控制任务级并发下载数量，默认 1，上限 4；较高数值可能增加带宽、CPU 和风控压力。
- `flv` 已移除，不作为当前视频容器选项。
